...
from odoo.exceptions import UserError, ValidationError # Importa ValidationError
import logging # Importa logging

_logger = logging.getLogger(__name__) # Inicializa o logger
...
class PesquisaAiiaSearchWizard(models.TransientModel):
    ...
    def action_start_search(self):
        ...
        try:
            ...
            _logger.info(f"Pesquisa iniciada via Wizard. Search ID: {search_id}")

            # Opcional: retornar ação para abrir a pesquisa criada ou a lista
            # Por enquanto, apenas fecha o wizard
            return {'type': 'ir.actions.act_window_close'}

        except (UserError, ValidationError) as e: # Agora ValidationError é conhecido
